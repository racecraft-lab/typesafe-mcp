...
description: >
  ...
  Also use to make a judgment right now with the `evaluate` MCP tool this
  plugin ships, instead of writing an integration first.
  Read live docs and cookbooks to find useful patterns and discover new combinations.
---

<!--
Adapted from TypeSafe AI's official agent skill.

  Source:  https://github.com/typesafe-ai/skills  (plugin `typesafe`, version 0.5.7)
...
building an application, and wrong when a judgment is needed in this session
and the `evaluate` tool is already connected. Shipping both unchanged leaves
the agent choosing between them with nothing to go on.

Install this plugin *instead of* the official `typesafe` plugin, not alongside
it: two skills named typesafe-ai would collide.
-->
...
The two share everything below: how to pick a primitive, how to structure
state, and how to compose answers apply either way. A question worth sending
through the tool is designed the same as one worth putting in code.

**The tool sends the state and questions you pass to an external provider, and
the call is billed.** The read-only annotation means it changes nothing on the
machine, not that it is free or private. Do not put a secret in `state` to have
a judgment made about it.

Which backend the tool uses is fixed by the operator's `JEV_PROVIDER` setting,
not by anything you pass. The server's own instructions name it at connect time.
